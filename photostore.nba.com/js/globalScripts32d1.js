	...
	var autoPromptTimerId = -99;
    var autoPromptTimestamp = new Date();
    var autoPromptPreviousTimestamp = autoPromptTimestamp.getTime();
	var autoPromptInputId = '';
	var autoPromptContainerId = '';
    ...
    function toggleMobileNavigationDiv(targetAction)
    {
        ...
        if(document.getElementById('mobileNavigationDiv').style.display != 'inline-block')
        {
			...
			if(targetAction == 'showProducts')
			{
				showMobileMenuProducts();
			}
        }
        else
        {
	...
	function showMobileMenuProducts()
	{
		var menus = document.getElementsByClassName('mobileNavigationListContainer');

		for(var i = 0; i < menus.length; i++)
		{
			if(menus[i].id == 'mobileNavigationMenuShop')
			{
				menus[i].style.display = 'inline-block';
			}
			else
    ...
    function showMobileMenu2023(e)
    {
		e = e || window.event;
		
		var targetElement = e.currentTarget;

		if(targetElement.hasAttribute('target-mobile-menu'))
		{
			var targeMobileMenuId = targetElement.getAttribute('target-mobile-menu');
		
			var menus = document.getElementsByClassName('mobileNavigationListContainer');

			for(var i = 0; i < menus.length; i++)
			{
				if(menus[i].id == targeMobileMenuId)
				{
					menus[i].style.display = 'inline-block';
				}
				else
    ...
    function toggleMobileSubMenu(menuId)
    {
        if(document.getElementById(menuId).style.display != 'inline-block')
        {
            ...
            {
                menus[i].style.display = 'none';
				menuParent = menus[i].parentElement;
				
				if(menuParent)
				{
					menuParent.setAttribute('data-expanded', 0);
				}
            }

            document.getElementById(menuId).style.display = 'inline-block';
			
			menuParent = document.getElementById(menuId).parentElement;
			
			if(menuParent)
			{
				menuParent.setAttribute('data-expanded', 1);
			}			
    ...
    function toggleMobileSubMenu2023(e)
    {
		e = e || window.event;
		
		var targetElement = e.currentTarget;

		if(targetElement.hasAttribute('target-mobile-sub-menu'))
		{
			var targeMobileSubMenuId = targetElement.getAttribute('target-mobile-sub-menu');
		
			if(document.getElementById(targeMobileSubMenuId).style.display != 'inline-block')
			{
				var menus = document.getElementsByClassName('mobileNavigationMenuSubList');
				...
				var menus = document.getElementsByClassName('mobileNavigationLinkDownArrow');

				for(var i = 0; i < menus.length; i++)
				{
					if(menus[i] !== targetElement)
					{
						menus[i].setAttribute('data-expanded', 0);
					}
				}

				targetElement.setAttribute('data-expanded', 1);
				document.getElementById(targeMobileSubMenuId).style.display = 'inline-block';
			}
			else
			{
				var menus = document.getElementsByClassName('mobileNavigationLinkDownArrow');
				...
				targetElement.setAttribute('data-expanded', 0);
				document.getElementById(targeMobileSubMenuId).style.display = 'none';
			}	
    ...
    function selectSearchType2023(e)
    {
		e = e || window.event;
		
		var targetElement = e.currentTarget;

		var searchTypeId = targetElement.getAttribute('data-search-type');
		var searchTypeName = targetElement.getAttribute('data-display-search-type');
    ...
    function globalAddToCartCompleteProductSku()
    {
		...
		// Pinterest
		if(simpleEcommerceId && typeof pintrk == 'function')
		{
		  pintrk('track', 'AddToCart', {
			value: simpleEcommercePrice,
			order_quantity: 1,
			currency: 'USD',
				line_items: [
					{
						product_id: simpleEcommerceId,
					},
				],
		  });
		}		
        ...
        httpobject[localvalue].onreadystatechange = function(){
            ...
            if(httpobject[localvalue].readyState == 4 && httpobject[localvalue].status == 200)
            {
                ...
                if(response[1])
                {
                    ...
                    if(false && responseData.hasOwnProperty('googleAnalyticsData'))
                    {
                        window.dataLayer = window.dataLayer || [];
                        window.dataLayer.push(responseData.googleAnalyticsData);
                    }
					
                    if(responseData && responseData.hasOwnProperty('ga4'))
                    {
                        window.dataLayer = window.dataLayer || [];
						window.dataLayer.push(responseData.ga4);
                    }					
        ...
        httpobject[localvalue].onreadystatechange = function(){
            ...
            if(httpobject[localvalue].readyState == 4 && httpobject[localvalue].status == 200)
            {
                ...
                if(response[1])
                {
                    ...
                    if(false && responseData && responseData.hasOwnProperty('googleAnalyticsData'))
                    {
                        window.dataLayer = window.dataLayer || [];
						window.dataLayer.push(responseData.googleAnalyticsData);
                    }
	...
	function positionAutoPromptDiv()
	{		
		var sourceElement = document.getElementById(autoPromptContainerId);
	
        if(!document.getElementById('autoPromptDiv'))
        {
            // Create the auto-prompt div.
            var newDiv = document.createElement('div');
            //var cssStyle = 'position: fixed; box-sizing: border-box; z-index: 5000; left: 0px; top: 0px; width: 10px; height: 10px; background-color: #000000; visibility: hidden; opacity: 0.50; filter: alpha(opacity=50);';
            newDiv.id = 'autoPromptDiv';
            //newDiv.setAttribute('style',cssStyle);
            //newDiv.style.cssText = cssstyle;
			newDiv.setAttribute('data-container-id',autoPromptContainerId);
            sourceElement.appendChild(newDiv);
        }
		else
		{
			if(document.getElementById('autoPromptDiv').getAttribute('data-container-id') != autoPromptContainerId)
			{
				sourceElement.appendChild(document.getElementById('autoPromptDiv'));
			}
		}
	
		var targetElement = document.getElementById('autoPromptDiv');

        var sourceWidth = document.defaultView.getComputedStyle(sourceElement,'').getPropertyValue('width');
        var sourceHeight = document.defaultView.getComputedStyle(sourceElement,'').getPropertyValue('height');		

		targetElement.style.width = sourceWidth;
		targetElement.style.top = sourceHeight;
	}
	
	function hideAutoPromptDiv()
	{
		document.getElementById('autoPromptDiv').innerHTML = '';
		document.getElementById('autoPromptDiv').style.display = 'none';
		window.removeEventListener('click', checkAutoPromptClick);
	}
	
	function showAutoPromptDiv()
	{
		document.getElementById('autoPromptDiv').style.display = 'inline-block';
		window.addEventListener('click', checkAutoPromptClick);
	}	
	
	function checkAutoPromptClick()
	{
		...
		if(document.getElementById(activeElementId).getAttribute('data-flag-search-input') != 1)
		{
			hideAutoPromptDiv();
			return;
		}
	...
	function autoPromptSearchTerms(e)
	{
		if(e == null)
		{
			e = window.event;
		...
		autoPromptInputId = eventSourceId;
		autoPromptContainerId = document.getElementById(eventSourceId).getAttribute('data-container-id');
		
		//console.log(autoPromptInputId + ' - ' + autoPromptContainerId);		

		if(document.getElementById('searchFormHeader'))
		{
			var flagArtistPage = document.getElementById('searchFormHeader').getAttribute('data-artist-page');
        ...
        if(autoPromptTimerId != -99)
        {
            clearTimeout(autoPromptTimerId);
        }		
	
		positionAutoPromptDiv();
	
        var searchTerm = document.getElementById(eventSourceId).value;

        var stamp = new Date();
        var currentTimestamp = stamp.getTime();

		var elapsedTime = (currentTimestamp - autoPromptPreviousTimestamp) / 1000;
        autoPromptPreviousTimestamp = currentTimestamp;         
        ...
        {       
            autoPromptTimerId = setTimeout(autoPromptSearchTerms,750,e);
            return;
        }		
		...
		if(searchTerm.length >= 3)
		{
			if(document.getElementById('autoPromptDiv'))
			{
				...
				httpobject[localvalue].onreadystatechange = function(){
					...
					if(httpobject[localvalue].readyState == 4 && httpobject[localvalue].status == 200)
					{
						if(false)
						{
							...
							var autoPromptResults = response.results;
							updateAutoPromptResults(autoPromptResults);
						}
						
						var response = httpobject[localvalue].responseText.split("<EXTRAINSTRUCTIONS>");
						var numResults = response[0];
						...
						showAutoPromptDiv();
						document.getElementById('autoPromptDiv').innerHTML = responseData;
						
						if(true)
						{
							var elements = document.getElementsByClassName('autoPromptResultButton');
							
							for(var i = 0; i < elements.length; i ++)
							{
								elements[i].addEventListener('click', handleAutoPromptClick, true);
							}
						}
					}
				...
				var url = "/queries/querySearchAutoPrompt.php";
				var poststring = "searchTerm=" + encodeURIComponent(searchTerm);
	...
	// Update the auto-prompt results.
	function updateAutoPromptResults(autoPromptResults)
	{
		var autoPromptResultButtons = document.getElementById('autoPromptDiv').getElementsByTagName('button');
		
		for(var i = 0; i < autoPromptResultButtons.length; i ++)
		{
			autoPromptResultButtons[i].parentNode.removeChild(autoPromptResultButtons[i]);
			i --;
		}

		for(var i = 0; i < autoPromptResults.length; i ++)
		{
			var newDiv = document.createElement('button');
			newDiv.type ='button';
			newDiv.id = 'autoPromptResultButton_' + i;
			newDiv.className = 'autoPromptResultButton';
			newDiv.innerHTML = autoPromptResults[i];
			newDiv.onclick = function() {alert('hello')};

			document.getElementById('autoPromptDiv').appendChild(newDiv);
		}
	...
	{
		hideAutoPromptDiv();
		//document.getElementById('searchFormHeaderInput').value = searchTerm;
		//document.getElementById('searchFormHeader').submit();
		
		document.getElementById(autoPromptInputId).value = searchTerm;
		document.getElementById(autoPromptContainerId).submit();		
		
	}
	
	function handleAutoPromptClick(e)
	{
		e = e || window.event;
		
		var targetElement = e.currentTarget;
		
		if(targetElement.hasAttribute('data-search-term'))
		{
			var searchTerm = targetElement.getAttribute('data-search-term');
			document.getElementById(autoPromptInputId).value = searchTerm;
			document.getElementById(autoPromptContainerId).submit();				
		}
		
		if(targetElement.hasAttribute('data-target-url'))
		{
			var targetUrl = targetElement.getAttribute('data-target-url');
			goToUrl(targetUrl);				
		}		
	}
	...
	function selectMobileNavigationSearchType2023(e)
	{
		e = e || window.event;
		
		var targetElement = e.currentTarget;

		if(targetElement.hasAttribute('data-search-type'))
		{
			var newSearchType = targetElement.getAttribute('data-search-type');
			var newSearchTypeName = document.getElementById('mobileNavigationSearchType-' + newSearchType).innerHTML;

			document.getElementById('mobileNavigationSearchType').value = newSearchType;
			document.getElementById('mobileNavigationSearchTypeName').innerHTML = newSearchTypeName;
	...
	function createGlobalForegroundAndBackgroundDiv2()
    {
        var newdiv;
        var cssstyle;

        if(!document.getElementById('globalBackgroundDiv2'))
        {
            // Background Div
            newdiv = document.createElement('div');
            cssstyle = 'position: fixed; box-sizing: border-box; z-index: 5000; left: 0px; top: 0px; width: 10px; height: 10px; background-color: #000000; visibility: hidden; opacity: 0.50; filter: alpha(opacity=50);';
            newdiv.id = 'globalBackgroundDiv2';
            newdiv.setAttribute('style',cssstyle);
            newdiv.style.cssText = cssstyle;
            document.body.appendChild(newdiv);
        }

        if(!document.getElementById('globalForegroundDiv2'))
        {
            // Foreground Div
            newdiv = document.createElement('div');
            cssstyle = 'position: fixed; box-sizing: border-box; z-index: 5001; left: 0px; top: 0px; width: 10px; height: 10px; overflow: auto; padding: 0px; border: 1px solid #666666; background-color: #FFFFFF; visibility: hidden; box-shadow: 0px 0px 50px #000000; text-align: center;';
            newdiv.id = 'globalForegroundDiv2';
            newdiv.setAttribute('style',cssstyle);
            newdiv.style.cssText = cssstyle;
            document.body.appendChild(newdiv);
    ...
    function showGlobalForegroundAndBackgroundDiv2()
    {
        var backgroundDiv = document.getElementById('globalBackgroundDiv2');
        var foregroundDiv = document.getElementById('globalForegroundDiv2');
	...
	function showGlobalDiscountMessageScrollUp()
    {
		if(document.getElementById('globalForegroundDiv') && document.getElementById('globalForegroundDiv').style.visibility == 'visible')
		{
			return;
		}
		
		if(document.getElementById('buttonShowDiscountPopUp'))
		{
			document.getElementById('buttonShowDiscountPopUp').style.display = 'none';
		}
		...
		backgroundDiv.style.zIndex = 100000;
		foregroundDiv.style.zIndex = 100001;
		...
		if(entranceType == 'scroll')
		{
            var cssStyle = 'position: fixed; box-sizing: border-box; left: 0px; top: 0px; width: 100%; height: 100%; background-color: #000000; z-index: 10000; opacity: 0.20; visibility: visible;';
            backgroundDiv.style.cssText = cssStyle;	
			backgroundDiv.onclick = function() {hideGlobalDiscountMessagePopUp()};
			
			var targetPercentage = 0.50;
			...
			var heightTarget = parseInt(Math.min(heightClient * targetPercentage, 500)) + 'px';			
			...
			foregroundDiv.style.overflow = 'hidden';
			foregroundDiv.style.visibility = 'visible';
			foregroundDiv.style.zIndex = 10001;

			foregroundDiv.style.left = '0px';
			foregroundDiv.style.top = 'auto';
			foregroundDiv.style.bottom = '0px';
			foregroundDiv.style.height = heightTarget;			
        }
		
		if(entranceType == 'popup')
		{
			var backgroundDiv = document.getElementById('globalBackgroundDiv');

            var cssStyle = 'position: fixed; box-sizing: border-box; left: 0px; top: 0px; width: 100%; height: 100%; background-color: #000000; z-index: 10000; opacity: 0.20;';
            backgroundDiv.style.cssText = cssStyle;			
			backgroundDiv.onclick = function() {hideGlobalDiscountMessagePopUp()};			

			var cssStyle = "position: fixed; display: inline-flex; flex-direction: column; flex-wrap: nowrap; justify-content: center; align-items: center; box-sizing: border-box; z-index: 10001; top: 0px; left: 0px; width: 100%; height: 100%;";
			foregroundDiv.style.cssText = cssStyle;
			foregroundDiv.style.visibility = 'visible';
			foregroundDiv.onclick = function() {hideGlobalDiscountMessagePopUp()};		
        ...
        var url = '/queries/queryGlobalDiscountMessageScrollUp.php';
        var postString = 'action=refresh&entranceType=' + entranceType;
    ...
    function globalCheckForEnterDiscountMessageScrollUp(e)
    {
        if(e.keyCode === 13)
        {
            e.preventDefault();
            ...
            globalSubmitEmailDiscountMessageScrollUp();
        }
    }


    function globalSubmitEmailDiscountMessageScrollUp()
    {

        var localvalue = requestcount;
        var email = document.getElementById('discountMessageEmail').value;
        ...
        document.getElementById('submitDiscountMessageEmailDiv').innerHTML = "<img src='/assets/images/Searching.gif' style='display: inline-block; padding-top: 5px;'>";
        ...
        var url = '/queries/queryGlobalDiscountMessageScrollUp.php';
        var postString = 'action=submit&email=' + email + '&sourcePage=' + encodeURIComponent(window.location.pathname + window.location.search) + '&entranceType=' + entranceType;
    ...
    function hideGlobalDiscountMessagePopUp()
    {
		if(document.getElementById('popUpDiscountOuterDiv') && document.getElementById('popUpDiscountOuterDiv').getAttribute('data-email-sent') == 0 && document.getElementById('buttonShowDiscountPopUp'))
		{
			document.getElementById('buttonShowDiscountPopUp').style.display = 'inline-block';
			document.getElementById('buttonShowDiscountPopUp').style.width = '100px';
			document.getElementById('buttonShowDiscountPopUp').style.height = '100px';		
		
			// This forces the browser to recognize the element's initial size before changing it.
			void document.getElementById('buttonShowDiscountPopUp').offsetWidth;
		
			document.getElementById('buttonShowDiscountPopUp').style.width = '30px';
			document.getElementById('buttonShowDiscountPopUp').style.height = '30px';			
    ...
    function hideGlobalForegroundAndBackgroundDiv2()
    {
        document.getElementById('globalBackgroundDiv2').style.visibility = 'hidden';
        document.getElementById('globalForegroundDiv2').style.visibility = 'hidden';
    }
    ...
    function goToUrl(url)
    {
		if(true)
		{
			...
			var targetUrl = '/queries/queryGoToUrl.php';
			var postString = 'url=' + encodeURIComponent(url);

			httpobject[localvalue].open('post',targetUrl,true);
			httpobject[localvalue].setRequestHeader('content-type', 'application/x-www-form-urlencoded');
			httpobject[localvalue].send(postString);
	...
	function goToUrl2023(e)
	{
		...
		if(targetElement.hasAttribute('data-target-url'))
		{
			var url = targetElement.getAttribute('data-target-url');
	...
	//window.addEventListener('globalScriptLoaded', function(){alert('loaded')});
	
	
	var flagGlobalScriptLoaded = true;
	var newEvent = new Event('globalScriptLoaded');
